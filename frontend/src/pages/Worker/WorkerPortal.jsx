...
export default function WorkerDashboard() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('workerTheme', theme);
  }, [theme]);
...
============================================
  ...
  return (
    <div className={`worker-dashboard ${theme}`}>
      <section className="worker-hero">
        <div className="worker-hero-copy">
          <span className="worker-eyebrow">SUPPORTDESK / FIELD OPS</span>
          <h1>Work orders, <span>in motion.</span></h1>
          <p>Stay on top of assigned jobs, update progress from the field, and close every task with confidence.</p>
          <div className="worker-hero-meta"><span className="worker-live-dot" /> Field network online <i /> {pendingTasks} tasks waiting</div>
        </div>
        <div className="worker-scene" aria-hidden="true">
          <div className="worker-orbit worker-orbit-one" />
          <div className="worker-orbit worker-orbit-two" />
          <div className="worker-workorder worker-workorder-back" />
          <div className="worker-workorder worker-workorder-front">
            <small>WORK ORDER / ACTIVE</small>
            <strong>Site visit queued</strong>
            <span className="worker-scene-line" /><span className="worker-scene-line short" />
            <b><i /> TECHNICIAN READY</b>
          </div>
          <span className="worker-scene-tag worker-tag-sla">ETA 18 MIN</span>
          <span className="worker-scene-tag worker-tag-scan">SCAN READY</span>
        </div>
      </section>
      ...
      <div className="worker-header">
        ...
        <div className="worker-header-right">
          ...
          <div className={`availability-toggle ${isAvailable ? 'available' : 'unavailable'}`} onClick={updateAvailability}>
      ...
      <div className="worker-tabs">
        ...
        <button className={`tab-btn ${activeTab === 'completed' ? 'active' : ''}`}
          onClick={() => { setActiveTab('completed'); setFilter('all'); setSearchTerm(''); }}>
          ✅ Completed ({completedTasks})
        </button>
        <button className={`tab-btn ${activeTab === 'rejected' ? 'active' : ''}`}
      ...
      {activeTab !== 'analytics' && activeTab !== 'settings' && (
        <div className="worker-filters">
          ...
          <div className="worker-actions">
            <button className="btn-report" onClick={() => setShowReportModal(true)}>📊 Report</button>
            <button className="btn-refresh" onClick={() => { fetchTasks(); fetchStats(); }}>🔄 Refresh</button>
          </div>
        </div>
      )}
            ...
            <div className="empty-state">
              ...
              <div className="empty-state-text">
                {searchTerm ? 'No tasks match your search' : 
                 ...
                 activeTab === 'rejected' ? 'No rejected tasks' : 'You have no tasks assigned'}
              </div>
            </div>
          ) : (
                ...
                <div key={task._id} className="worker-task-card">
                  ...
                  <div className="task-card-body">
                    ...
                    <div className="task-meta">
                      <span className="task-meta-item"><span className="meta-icon">👤</span> {task.customerId?.name}</span>
                      <span className="task-meta-item"><span className="meta-icon">📂</span> {task.category}</span>
                      <span className="task-meta-item"><span className="meta-icon">📅</span> {new Date(task.createdAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  ...
                  <div className="task-card-footer">
                    <div className="task-assignee">
                      {task.workerId ? (
                        <div className="assignee-info">
                          <span className="assignee-avatar">{task.workerId.name?.charAt(0).toUpperCase()}</span>
                    ...
                    <div className="task-actions">
                      {(task.status === 'Assigned' || task.status === 'New') && (
                        <>
                          <button onClick={() => handleStatusUpdate(task._id, 'Accepted')} className="btn-accept">✅ Accept</button>
                          <button onClick={() => handleStatusUpdate(task._id, 'Rejected')} className="btn-reject">❌ Reject</button>
                          <button onClick={() => handleStatusUpdate(task._id, 'In Progress')} className="btn-progress">⏳ Start</button>
                        </>
                      )}
                      {task.status === 'In Progress' && (
                        <button onClick={() => handleStatusUpdate(task._id, 'Completed')} className="btn-complete">✅ Complete</button>
                      )}
                      {(task.status === 'Completed' || task.status === 'Rejected' || task.status === 'Cancelled') && (
                        <span className="btn-disabled">{task.status === 'Completed' ? '✅ Done' : task.status === 'Rejected' ? '❌ Rejected' : '🚫 Cancelled'}</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="analytics-grid">
              ...
              {Object.entries(chartData.status).map(([status, count]) => (
                <div key={status} className="chart-bar-item">
                  ...
                  <div className="chart-bar-track">
                    <div className={`chart-bar-fill ${getStatusClass(status)}`} style={{ width: `${totalTasks > 0 ? (count / totalTasks) * 100 : 0}%` }} />
                  </div>
            ...
            <div className="analytics-card">
              ...
              {Object.entries(chartData.priority).map(([priority, count]) => (
                <div key={priority} className="chart-bar-item">
                  ...
                  <div className="chart-bar-track">
                    <div className={`chart-bar-fill ${getPriorityClass(priority)}`} style={{ width: `${totalTasks > 0 ? (count / totalTasks) * 100 : 0}%` }} />
                  </div>
                  <span className="chart-count">{count}</span>
                </div>
              ))}
            ...
            <div className="analytics-card full-width">
              ...
              <div className="performance-grid">
                <div className="perf-item"><span className="perf-label">Total</span><span className="perf-value">{totalTasks}</span></div>
                <div className="perf-item"><span className="perf-label">Completion Rate</span><span className="perf-value">{totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0}%</span></div>
                <div className="perf-item"><span className="perf-label">Avg Rating</span><span className="perf-value" style={{ color: '#eab308' }}>⭐ {user?.rating?.average || 0}/5</span></div>
                <div className="perf-item"><span className="perf-label">Reviews</span><span className="perf-value">{user?.rating?.count || 0}</span></div>
                <div className="perf-item"><span className="perf-label">Pending</span><span className="perf-value" style={{ color: '#ef4444' }}>{pendingTasks}</span></div>
                <div className="perf-item"><span className="perf-label">Status</span><span className="perf-value" style={{ color: isAvailable ? '#22c55e' : '#ef4444' }}>{isAvailable ? '✅ Available' : '⛔ Unavailable'}</span></div>
              </div>
            </div>
            ...
            <div className="analytics-card full-width">
              ...
              <div className="report-buttons">
                <button className="btn-download" onClick={() => downloadReport('json')}>📊 JSON</button>
                <button className="btn-download-csv" onClick={() => downloadReport('csv')}>📊 CSV</button>
                <button className="btn-download-text" onClick={() => downloadReport('text')}>📄 Text</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="settings-grid">
            <div className="settings-card availability-card">
              ...
              <div className="availability-status">
                ...
                <p className="status-hint">{isAvailable ? 'You will receive new tasks' : 'You will not receive new tasks'}</p>
              </div>
            </div>
            ...
            <div className="settings-card profile-card">
              ...
              <button className="settings-btn profile-btn" onClick={handleUpdateProfile}>💾 Update</button>
            </div>
            ...
            <div className="settings-card security-card">
              ...
              <button className="settings-btn password-btn" onClick={handleChangePassword}>🔑 Change</button>
            </div>

            <div className="settings-card system-card">
              <h3>📊 System</h3>
              <div className="system-stat"><span>Total Tasks</span><span className="system-stat-value">{totalTasks}</span></div>
              <div className="system-stat"><span>Completed</span><span className="system-stat-value success">{completedTasks}</span></div>
              <div className="system-stat"><span>In Progress</span><span className="system-stat-value warning">{inProgressTasks}</span></div>
              <div className="system-stat"><span>Rating</span><span className="system-stat-value" style={{ color: '#eab308' }}>⭐ {user?.rating?.average || 0}/5</span></div>
              <div className="system-stat"><span>Status</span><span className={`system-stat-value ${isAvailable ? 'success' : 'danger'}`}>{isAvailable ? '✅ Available' : '⛔ Unavailable'}</span></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowReportModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p className="modal-subtitle">Select format:</p>
              <div className="report-options">
                <button className="report-option" onClick={() => downloadReport('json')} disabled={reportLoading}>
                  <span className="report-icon">📄</span> JSON <span className="report-desc">Detailed data</span>
                </button>
                <button className="report-option" onClick={() => downloadReport('csv')} disabled={reportLoading}>
                  <span className="report-icon">📊</span> CSV <span className="report-desc">Spreadsheet</span>
                </button>
                <button className="report-option" onClick={() => downloadReport('text')} disabled={reportLoading}>
                  <span className="report-icon">📋</span> Text <span className="report-desc">Printable</span>
                </button>
              </div>
